    ...
    var body: some View {
        List{
            ForEach(productList.indices, id: \.self) { index in
                ...
                NavigationLink(destination: SingleContentView(dataModel: $productList[index], isShowItems: false).environmentObject(userData)) {
                    HStack{
...
//        }
        ...
        return (jsonContentData as! [[[[Int]]]]).enumerated().map { index, item in
            return EnterItem(name: "第\(index+1)种", matrix: item, usedBlock: [], isTaskComplete: false)
        }
    }
}
