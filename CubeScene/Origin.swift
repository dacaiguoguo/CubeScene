...
class Origin: SCNNode {
    ...
    init(length: CGFloat = 0.1, radiusRatio ratio: CGFloat = 0.002, color: (x: UIColor, y: UIColor, z: UIColor, origin: UIColor) = (.blue, .green, .red, .cyan)) {
        ...
        xAxisNode.simdWorldPosition = simd_float1(length)/2 * Axis.x.normal * -1  // 向相反方向移动
